...
/************************ Layering Functions ******************************/
void Adafruit_RA8875::enableTwoLayers(boolean on) {
  uint8_t temp = readReg(RA8875_LAYER_MODE);
  temp &= 0x7F; // isolate the 7th bit
  if(on) {
    temp |= RA8875_TWO_LAYERS;
  }
  writeReg(RA8875_LAYER_MODE, temp);
}

void Adafruit_RA8875::enableTransparentMode(boolean on, uint16_t transColor) {
  uint8_t temp = readReg(RA8875_LTPR0);
  temp &= 0xF8;
  uint8_t red, green, blue = 0;
  if(on) {
    temp |= RA8875_TRANS;
    red = (uint8_t)(transColor >> 11);
    green = (uint8_t)(transColor >> 5);
    blue = (uint8_t)(transColor);
  }
  writeReg(RA8875_LTPR0, temp);
  writeReg(RA8875_BGTR_RED, red);
  writeReg(RA8875_BGTR_GREEN, green);
  writeReg(RA8875_BGTR_BLUE, blue);
}

void Adafruit_RA8875::layerMode(uint8_t layer) {
  uint8_t temp = readReg(RA8875_LAYER_SELECT);
  temp &= 0xFE; // isolate the 0th bit
  if(layer == 2) {
    temp |= RA8875_L2_SELECT;
  }
  writeReg(RA8875_LAYER_SELECT, temp);
}

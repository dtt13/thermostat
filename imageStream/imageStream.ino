...
#define COMMAND_BUFFER_SIZE 740
...
void swapBuffers() {
//  char *read_addr = input_read;
//  char *write_addr = input_write;
...
//  Serial.println("buff_ptr_read addr: " + String((int) &buff_ptr_read));
  Serial1.readBytes(buff_ptr_write+1, 2);
  int packetSize = unpackNumber(buff_ptr_write, 1, 2);
  int bytesRead = 0;
  while(bytesRead < packetSize && Serial1.available() > 0) {
    bytesRead += Serial1.readBytes(buff_ptr_write + bytesRead + 3, packetSize-bytesRead);
  }
  swapBuffers();
  return bytesRead;
}
...
void processImage() {
  while(Serial1.available() > 0) {
    memset(buff_ptr_write, 0x00, COMMAND_BUFFER_SIZE);
//    int numBytes = Serial1.readBytesUntil('\n', ret, COMMAND_BUFFER_SIZE);
    int numBytes;
    Serial1.readBytes(buff_ptr_write, 1);
//    Serial.println("number of bytes: " + String(numBytes));
    switch(buff_ptr_write[0]) {
      case STREAM_IMAGE:
        Serial.println("stream image");
        numBytes = readPacket();
//        Serial.println(String(numBytes));
//        printData(buff_ptr_read, numBytes + 2);
//        convert2PixelData(pixelData, ret + 11, numBytes - 8);
//        sc->drawImage(pixelData, (numBytes - 8) / 2, unpackNumber(ret, 3, 2), unpackNumber(ret, 5, 2),
